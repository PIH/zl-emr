...
update temp_mh_encounters set speech_normal = 
	CASE
		when obs_value_coded_list_from_temp(encounter_id,'PIH','14286',@locale) = @normal then 1
		when obs_value_coded_list_from_temp(encounter_id,'PIH','14286',@locale) = @abnormal then 0
	END;

update temp_mh_encounters set cognitive_function_normal = 
	CASE
		when obs_value_coded_list_from_temp(encounter_id,'PIH','20722',@locale) = @normal then 1
		when obs_value_coded_list_from_temp(encounter_id,'PIH','20722',@locale) = @abnormal then 0
	END;
